...
void AcidSynthAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    voice.setParams (wave, cutoff, res, envmod, decay, accent, glide, drive, sat, subMix, unison, unisonSpread, gain);
    ...
    if (reverbMix > 0.0001f)
    {
        ...
        reverb.processStereo (buffer.getWritePointer (0),
                              numCh > 1 ? buffer.getWritePointer (1) : buffer.getWritePointer (0),
                              numSamples);
    }
}
